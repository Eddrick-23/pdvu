...
TEST(KittyProtocolUtils, Base64Encoding) {
  struct {
    std::string input;
    std::string expected;
  } test_cases[] = {{"Hello", "SGVsbG8="}, {"World", "V29ybGQ="}, {"", ""}, {"foo", "Zm9v"}};
...
TEST(KittyProtocolUtils, BlackPixel3x3Encoding) {
  ...
  struct TestCase {
    ...
    std::string expected;  // base64 of RGBA bytes [0,0,0,alpha]
      ...
      {0, concat_strings("AAAAAAAAAAAAAAAA", 3)},   {25, concat_strings("AAAAPwAAAD8AAAA/", 3)},
      {50, concat_strings("AAAAfwAAAH8AAAB/", 3)},  {75, concat_strings("AAAAvwAAAL8AAAC/", 3)},
...
TEST(KittyProtocol, ImageSequence_StructureAndParameters_Transmit) {
  std::string result = get_image_sequence("testpath", 1, 600, 800, 0, 0, 0, 0, "shm", true);
...
TEST(KittyProtocol, KittyProtocol_ImageSequence_StructureAndParameters_NoTransmit) {
  std::string result = get_image_sequence("testpath", 1, 600, 800, 0, 0, 0, 0, "shm", false);
...
TEST(KittyProtocol, ImageSequence_TransmissionFormat) {
  auto result_shm = get_image_sequence("testfile", 1, 100, 100, 0, 0, 0, 0, "shm", true);
  auto result_tempfile = get_image_sequence("testfile", 1, 100, 100, 0, 0, 0, 0, "tempfile", true);
...
TEST(KittyProtocol, ImageSequence_OptionalImageDimensions) {
  auto result = get_image_sequence("testfile", 1, 100, 100, 0, 0, 0, 0, "shm", true, 50, 50);
...
TEST(KittyProtocol, PathEncoding) {
  ...
  for (auto& [input, expected] : test_paths) {
    auto result = get_image_sequence(input, 1, 100, 100, 0, 0, 0, 0, "shm", true);
