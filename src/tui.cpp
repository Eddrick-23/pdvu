...
namespace TUI {
    ...
    std::string bottom_bar(const TermSize& ts) {
        const std::string text = std::format("SEARCH: ctrl/cmd + f {} GO TO PAGE: g {} NAVIGATE: <- -> {} QUIT : q {} Help: ?",
        ...
        result += terminal::move_cursor(ts.height, 1);  // move to last row
        result += add_centered(ts.height, ts.width, text, visible_length(text)); // text for bottom bar
    ...
    std::string guard_message(const TermSize& ts) {
        ...
        result += add_centered(center_row, ts.width, current_dimensions, visible_length(current_dimensions));

        result += green;
        result += add_centered(center_row + 2, ts.width, required_dimensions, visible_length(required_dimensions));
        result += "\x1b[0m";       // Reset
        return result;
    }
    // TODO
