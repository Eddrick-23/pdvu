...
std::optional<PageCacheData> RenderEngine::try_page_cache(const RenderRequest& req,
    ...
    if (!shm_ptr && !tempfile_ptr) { // our cache stores empty data
        ...
        // TODO change this to update error message to display on main page  or fall through and rerender
        // update lru_cache to remove this entry since its carrying null data
    }
    return data;
}